...
DB_PCM = {
    "RT22HC (Bio-based)": {"kf": 0.2, "Tm": 22.0, "rho": 800},
    "RT35HC (Bio-based)": {"kf": 0.2, "Tm": 35.0, "rho": 820},
    "RT44HC (Parafina)": {"kf": 0.2, "Tm": 44.0, "rho": 800},
    "RT55 (Parafina)": {"kf": 0.2, "Tm": 55.0, "rho": 880},
    "Ácido Láurico": {"kf": 0.15, "Tm": 43.0, "rho": 940},
...
with st.sidebar:
    ...
    # Sección 2: Geometría e IA
    with st.expander("📏 GEOMETRÍA E IA VISION", expanded=False):
        modo_geo = st.radio("Método de medida", ["Manual", "IA Vision"])
        if modo_geo == "Manual":
            ...
            foto = st.file_uploader("Subir foto cenital", type=['jpg', 'png'])
            if foto:
                st.image(Image.open(foto), use_column_width=True)
                st.success("IA: Referencia detectada")
                d_final = st.number_input("Diámetro Detectado (mm)", value=25.4)
                l_final = st.number_input("Espesor Detectado (mm)", value=40.1)

    # Sección 3: Propiedades de la Espuma
    with st.expander("🧮 MICROESTRUCTURA", expanded=False):
...
# --- 6. CUERPO PRINCIPAL ---
st.title("🌡️ METALPHOAM 2 PRO")
st.write(f"Configuración actual: **{sel_m}** + **{sel_p}**")

t1, t2, t3 = st.tabs(["📄 Ficha de Muestra", "📊 Análisis Térmico", "📝 Generar Informe"])
...
with t2:
    st.subheader("Análisis de Ensayos")
...
with t3:
    st.subheader("Resumen para Memoria")
    resumen = f"Muestra de {sel_m} (ε={epsilon*100}%) con {sel_p}. ke={ke_res:.4f} W/mK."
    st.text_area("Texto para copiar:", resumen, height=100)
